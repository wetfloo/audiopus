...
//! does not know any types larger than `u32` and does not expect empty packets.
//!
...
//! empty and not longer than [`std::u32::MAX`].
//!
...
impl From<Bitrate> for u32 {
    fn from(bitrate: Bitrate) -> u32 {
        match bitrate {
...
impl TryFrom<u32> for Bitrate {
    ...
    fn try_from(value: u32) -> Result<Bitrate> {
        Ok(match value {
...
impl TryFrom<u32> for Application {
    type Error = Error;
...
impl TryFrom<u32> for Channels {
    ...
    fn try_from(value: u32) -> Result<Channels> {
        Ok(match value {
...
impl<'a, T> MutSignals<'a, T> {
    ...
    /// around a immutably borrowed buffer, we can safely cast `usize` to `u32`
    /// without worrying about `usize` being too large for `u32`.
